...
- **Step 5 backstop timeout on a HEALTHY, still-progressing container (slow first boot, not a fault).** Spawn dies with `Web did not become ready in ${WAIT_TIMEOUT}s` but `docker compose ps` shows web Up and the log still advancing (uv sync / migrate). Time the phases with `docker logs --timestamps`: on a cold per-session `uv_cache` volume the FIPS-mandated `--no-binary` source builds dominate — observed 2026-08-09: `Built cryptography==50.0.0` alone 4m33s, whole entrypoint ~6–7 min vs the 300s backstop. Markers: `.dev-credentials` absent (Step 6 never ran) + no registry row. Fix: do NOT nuke — let the entrypoint finish, then run the tail steps manually (Step 6 creds + `manage.py boot`, 6.4 passkey `--json`→`--import`, 6.5 `manage.py health --json`, registry append). Structural fix is a warm cache / published image (or a bigger backstop), not a code repair.
