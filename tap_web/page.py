...
def get_page_panels(page: Page) -> list[tuple[str, Panel]]:
    ...
    for edge in edges:
        ...
        try:
            ...
            logger.warning("USES_PANEL edge %s points to missing Panel entity %s", edge.pk, edge.to_entity_id)
...
def get_landing_page() -> Page | None:
    ...
    try:
        ...
        logger.warning("USES_LANDING_PAGE edge %s points to missing Page entity %s", edge.pk, edge.to_entity_id)
        return None
