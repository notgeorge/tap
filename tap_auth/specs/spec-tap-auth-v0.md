...
| req-tap-auth-policy-9 | Coverage Lint | Proposed | A build-time static lint (in-house AST scanner reusing the `test_log_site_ids` scanner + baseline-ratchet machinery) flags any service-layer write/read sink (`write_batch`, the Search/Gryphon executors, `grift_import`, the `_*_internal` write helpers) whose enclosing function lacks a `@requires_capability`/`authorized()` gate, plus direct graph-model ORM in disallowed zones (panels/views). Catches the forgotten-gate-by-a-privileged-actor case the runtime backstop cannot. Builds vs. Semgrep: the scanner enumerates graph-managed `BaseModel` subclasses at runtime (incl. plugin models) which a purely-syntactic tool cannot, and reuses the existing baseline-ratchet + pytest gate — revisit Semgrep only at a suite of rules. | |
...
- Plugin-defined internal / per-collector service actors. v0 runs **all** collectors as one shared `tap_collector` `program` actor (with a single shared collector capability bundle). Minting a distinct actor per collector is deferred to the **plugin refactor**, where plugins gain a first-class way to declare their own internal users/capabilities — that mechanism is the right home for it, not the auth sync or the boot system. We deliberately do **not** hardcode actors for the current collector set, since those collectors all migrate into plugins later. The demand for per-collector *identity* (attribution across multiple collectors) is real; the blocker is that there is no collector-self-registration-of-actor mechanism yet, which the plugin refactor provides. (`req-tap-auth-builtins` lists collector service actors as future built-ins.)
