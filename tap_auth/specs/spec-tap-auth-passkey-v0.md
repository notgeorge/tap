...
- **Assertion/registration verification (the load-bearing *enforcement* settings).** `py_webauthn`'s verify functions are insecure-by-omission — requesting a policy in the options does nothing unless the server *verifies* it in the response — so these are stated normatively rather than left to implementer discretion:
  ...
  - **Challenge lifecycle** — the server-side challenge is minted with ≥16 bytes of CSPRNG entropy (the library default is 64), **bound to the specific session + intended user**, given a short TTL, and consumed atomically (single-use) so a challenge minted for one ceremony cannot be redeemed in a parallel one. `py_webauthn` verifies the challenge value passed to it but does not store, bind, or expire it — that is entirely TAP's responsibility.
...
- **Conditional UI (autofill)** for usernameless login. Heed the Shopify caution: do **not** dangle passkey autofill at invitation-gated first visitors who cannot yet have a credential; present it only where a credential is likely to exist.
...
| req-tap-auth-passkey-webauthn-9 | Challenge Bound & Single-Use | Proposed | The server-side challenge is CSPRNG (≥16B), bound to session+user, TTL'd, and consumed atomically; it cannot be redeemed in a parallel ceremony. | |
...
| req-tap-auth-passkey-enrollment-6 | Non-Enumerating Redemption | Proposed | All redemption failures collapse to one generic constant-time response; the enrollment page carries `no-store`/`noindex`/`no-referrer` and the secret rides the URL path, not the query string. | |
