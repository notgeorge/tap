...
def get_builtin_actor(builtin_key: str) -> AbstractUser:
    ...
    Raises `MissingActor` if it does not exist or is inactive — auth sync must
    run before any system-initiated service-layer write.
    """
    actor = get_user_model().objects.filter(tap_builtin_key=builtin_key, is_active=True).first()
    if actor is None:
        raise MissingActor(
