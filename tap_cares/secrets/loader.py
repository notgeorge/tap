...
def load_secrets(
    ...
    *,
    ...
    if root is None:
        logger.info("tap-cares secrets: no root configured; skipping load.")
        return []

    root_path = Path(root)
    if not root_path.exists():
        logger.info("tap-cares secrets: root %s does not exist; skipping load.", root_path)
        return []
    ...
    logger.info(
        "tap-cares secrets: loaded %d secret(s) from %s.",
        len(loaded),
        root_path,
    )
    return loaded
