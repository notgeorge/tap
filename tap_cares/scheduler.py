...
def _finalize_fire_skipped(
    ...
    if not result.success:
        logger.exception(
            "scheduler: SKIPPED patch failed for fire %s: %s",
            fire.entity_id,
            result.errors,
        )
...
def _finalize_fire_failed(
    ...
    if not result.success:
        logger.exception(
            "scheduler: FAILED patch failed for fire %s: %s",
            fire.entity_id,
            result.errors,
        )
    ...
    *,
    ...
    for schedule in enabled_schedules:
        ...
        try:
            ...
            logger.exception(
                "scheduler: Stage 1 failed for schedule %s slot %s",
                schedule.entity_id,
        ...
        try:
            ...
            try:
                ...
                logger.exception(
                    "scheduler: run_collection raised for schedule %s",
                    schedule.entity_id,
                )
            ...
            logger.exception(
                "scheduler: Stage 2 failed for fire %s",
                fire.entity_id,
            )
            try:
                _finalize_fire_failed(
                ...
                logger.exception(
                    "scheduler: even fire FAILED patch failed for %s",
                    fire.entity_id,
                )
