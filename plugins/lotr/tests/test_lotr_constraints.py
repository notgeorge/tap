...
def frodo():
    ...
    return create_entity("lotr__character", name="Frodo Baggins")


@pytest.fixture
def gandalf():
    """Create another character entity (Gandalf)."""
    return create_entity("lotr__character", name="Gandalf the Grey")


@pytest.fixture
def sauron():
    """Create an antagonist character (Sauron)."""
    return create_entity("lotr__character", name="Sauron")


@pytest.fixture
def the_shire():
    """Create a location entity (The Shire)."""
    return create_entity("lotr__location", name="The Shire")


@pytest.fixture
def mordor():
    """Create another location entity (Mordor)."""
    return create_entity("lotr__location", name="Mordor")


@pytest.fixture
def bag_end():
    """Create a sub-location entity (Bag End)."""
    return create_entity("lotr__location", name="Bag End")


@pytest.fixture
def one_ring():
    """Create an artifact entity (The One Ring)."""
    return create_entity("lotr__artifact", name="The One Ring")


@pytest.fixture
def sting():
    """Create another artifact entity (Sting)."""
    return create_entity("lotr__artifact", name="Sting")


@pytest.fixture
def hobbit():
    """Create a race entity (Hobbit)."""
    return create_entity("lotr__race", name="Hobbit")


@pytest.fixture
def elf():
    """Create another race entity (Elf)."""
    return create_entity("lotr__race", name="Elf")


@pytest.fixture
def fellowship():
    """Create a faction entity (The Fellowship)."""
    return create_entity("lotr__faction", name="The Fellowship of the Ring")


@pytest.fixture
def mordor_forces():
    """Create another faction entity (Mordor Forces)."""
    return create_entity("lotr__faction", name="Forces of Mordor")


@pytest.fixture
def rohan():
    """Create another faction entity (Rohan)."""
    return create_entity("lotr__faction", name="Riders of Rohan")


@pytest.fixture
def sentinel():
    """Create a sentinel entity (for wildcard tests)."""
    return create_entity("lotr__sentinel", name="The Watcher")


@pytest.fixture
def citadel():
    """Create a citadel entity (for blocked inbound tests)."""
    return create_entity("lotr__citadel", name="Minas Tirith Citadel")


@pytest.fixture
def wanderer():
    """Create a wanderer entity (unconstrained)."""
    return create_entity("lotr__wanderer", name="Tom Bombadil")
...
class TestBasicValidEdges:
    ...
    def test_character_wields_artifact(self, frodo, sting):
        ...
        edge = create_edge(frodo, sting, "WIELDS__lotr")
        assert edge.edge_type == "WIELDS__lotr"
    ...
    def test_character_located_in_location(self, frodo, the_shire):
        ...
        edge = create_edge(frodo, the_shire, "LOCATED_IN__lotr")
        assert edge.edge_type == "LOCATED_IN__lotr"

    def test_character_rules_location(self, sauron, mordor):
        """Character can RULES a location."""
        edge = create_edge(sauron, mordor, "RULES__lotr")
        assert edge.edge_type == "RULES__lotr"

    def test_character_belongs_to_race(self, frodo, hobbit):
        """Character can BELONGS_TO a race."""
        edge = create_edge(frodo, hobbit, "BELONGS_TO__lotr")
        assert edge.edge_type == "BELONGS_TO__lotr"

    def test_character_member_of_faction(self, frodo, fellowship):
        """Character can be MEMBER_OF a faction."""
        edge = create_edge(frodo, fellowship, "MEMBER_OF__lotr")
        assert edge.edge_type == "MEMBER_OF__lotr"

    def test_artifact_forged_in_location(self, one_ring, mordor):
        """Artifact can be FORGED_IN a location."""
        edge = create_edge(one_ring, mordor, "FORGED_IN__lotr")
        assert edge.edge_type == "FORGED_IN__lotr"
...
class TestSelfReferentialEdges:
    ...
    def test_location_contains_location(self, the_shire, bag_end):
        ...
        edge = create_edge(the_shire, bag_end, "CONTAINS__lotr")
        assert edge.edge_type == "CONTAINS__lotr"
    ...
    def test_multiple_containment_edges(self, the_shire, bag_end, mordor):
        ...
        edge1 = create_edge(the_shire, bag_end, "CONTAINS__lotr")
        # Mordor can also contain things
        mount_doom = create_entity("lotr__location", name="Mount Doom")
        edge2 = create_edge(mordor, mount_doom, "CONTAINS__lotr")
        assert edge1.edge_type == "CONTAINS__lotr"
        assert edge2.edge_type == "CONTAINS__lotr"
...
class TestBidirectionalEdges:
    ...
    def test_faction_allies_with_faction(self, fellowship, rohan):
        ...
        edge = create_edge(fellowship, rohan, "ALLIES_WITH__lotr")
        assert edge.edge_type == "ALLIES_WITH__lotr"

    def test_faction_enemies_with_faction(self, fellowship, mordor_forces):
        """Faction can ENEMIES_WITH another faction."""
        edge = create_edge(fellowship, mordor_forces, "ENEMIES_WITH__lotr")
        assert edge.edge_type == "ENEMIES_WITH__lotr"

    def test_bidirectional_alliance(self, fellowship, rohan):
        """Both factions can ally with each other (two edges)."""
        edge1 = create_edge(fellowship, rohan, "ALLIES_WITH__lotr")
        edge2 = create_edge(rohan, fellowship, "ALLIES_WITH__lotr")
...
class TestWildcardEdges:
    ...
    def test_sentinel_references_another_sentinel(self, sentinel):
        ...
        other_sentinel = create_entity("lotr__sentinel", name="Palantir")
        edge = create_edge(sentinel, other_sentinel, "REFERENCES__lotr")
        assert edge.edge_type == "REFERENCES__lotr"

    def test_sentinel_references_wanderer(self, sentinel, wanderer):
        """Sentinel -> wanderer works (wanderer has no inbound constraints)."""
        edge = create_edge(sentinel, wanderer, "REFERENCES__lotr")
        assert edge.edge_type == "REFERENCES__lotr"
    ...
    def test_sentinel_blocked_by_target_inbound_character(self, sentinel, frodo):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(sentinel, frodo, "REFERENCES__lotr")
        assert "lotr__character cannot receive 'REFERENCES__lotr' edges" in str(exc_info.value)
    ...
    def test_sentinel_blocked_by_target_inbound_location(self, sentinel, the_shire):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(sentinel, the_shire, "REFERENCES__lotr")
        assert "lotr__location cannot receive 'REFERENCES__lotr' edges" in str(exc_info.value)
    ...
    def test_sentinel_blocked_by_target_inbound_artifact(self, sentinel, one_ring):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(sentinel, one_ring, "REFERENCES__lotr")
        assert "lotr__artifact cannot receive 'REFERENCES__lotr' edges" in str(exc_info.value)
    ...
    def test_sentinel_blocked_by_target_inbound_faction(self, sentinel, fellowship):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(sentinel, fellowship, "REFERENCES__lotr")
        assert "lotr__faction cannot receive 'REFERENCES__lotr' edges" in str(exc_info.value)
    ...
    def test_sentinel_cannot_use_non_references_edge(self, sentinel, wanderer):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(sentinel, wanderer, "WIELDS__lotr")
        assert "cannot create 'WIELDS__lotr' edges" in str(exc_info.value)
...
class TestBlockedOutbound:
    ...
    def test_race_cannot_create_any_edge(self, hobbit, frodo):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(hobbit, frodo, "BELONGS_TO__lotr")
...
class TestBlockedInbound:
    ...
    def test_citadel_blocks_any_edge_type(self, wanderer, citadel):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(wanderer, citadel, "PROTECTS__lotr")
        assert "cannot receive any inbound edges" in str(exc_info.value)
    ...
    def test_constrained_source_fails_at_inbound_block(self, frodo, citadel):
        ...
        """
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(frodo, citadel, "LOCATED_IN__lotr")
    ...
    def test_citadel_can_still_create_edges(self, citadel, the_shire):
        ...
        edge = create_edge(citadel, the_shire, "PROTECTS__lotr")
        assert edge.edge_type == "PROTECTS__lotr"
...
class TestUnconstrainedEntity:
    ...
    def test_wanderer_to_wanderer_any_edge(self, wanderer):
        ...
        goldberry = create_entity("lotr__wanderer", name="Goldberry")
    ...
    def test_constrained_source_still_applies_outbound(self, frodo, wanderer):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(frodo, wanderer, "ALLIES_WITH__lotr")
        assert "cannot create 'ALLIES_WITH__lotr' edge to lotr__wanderer" in str(exc_info.value)
    ...
    def test_wanderer_blocked_by_target_inbound(self, wanderer, frodo):
        ...
        assert "lotr__character cannot receive 'MYSTERIOUS_LINK' edges" in str(exc_info.value)
    ...
    def test_wanderer_can_use_allowed_edge_to_constrained_target(self, wanderer, frodo):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(wanderer, frodo, "ALLIES_WITH__lotr")
        assert "lotr__character cannot receive 'ALLIES_WITH__lotr' edge from lotr__wanderer" in str(exc_info.value)
    ...
    def test_wanderer_multiple_edges_to_other_wanderer(self, wanderer):
        ...
        edge2 = create_edge(wanderer, goldberry, "PROTECTS__lotr")
        edge3 = create_edge(wanderer, goldberry, "TEACHES")
        assert edge1.edge_type == "LOVES"
        assert edge2.edge_type == "PROTECTS__lotr"
...
class TestInvalidEdgeType:
    ...
    def test_artifact_cannot_use_wields(self, one_ring, frodo):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(one_ring, frodo, "WIELDS__lotr")
        assert "cannot create 'WIELDS__lotr' edges" in str(exc_info.value)

    def test_location_cannot_use_member_of(self, the_shire, fellowship):
        """Location cannot use MEMBER_OF edge."""
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(the_shire, fellowship, "MEMBER_OF__lotr")
        assert "cannot create 'MEMBER_OF__lotr' edges" in str(exc_info.value)
...
class TestInvalidTargetType:
    ...
    def test_character_cannot_wield_location(self, frodo, the_shire):
        ...
        """
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(frodo, the_shire, "WIELDS__lotr")
        assert "cannot receive 'WIELDS__lotr' edges" in str(exc_info.value)

    def test_character_cannot_belong_to_faction(self, frodo, fellowship):
        """BELONGS_TO only works for race, not faction."""
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(frodo, fellowship, "BELONGS_TO__lotr")
        assert "cannot create 'BELONGS_TO__lotr' edge to lotr__faction" in str(exc_info.value)
    ...
    def test_artifact_cannot_forge_in_character(self, one_ring, frodo):
        ...
        """
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(one_ring, frodo, "FORGED_IN__lotr")
        assert "cannot receive 'FORGED_IN__lotr' edges" in str(exc_info.value)

    def test_faction_cannot_ally_with_character(self, fellowship, frodo):
        """Faction can ALLIES_WITH faction, not character."""
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(fellowship, frodo, "ALLIES_WITH__lotr")
        assert "cannot create 'ALLIES_WITH__lotr' edge to lotr__character" in str(exc_info.value)
...
class TestInboundConstraintViolation:
    ...
    def test_location_cannot_receive_wields(self, frodo, the_shire):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(frodo, the_shire, "WIELDS__lotr")
        assert "WIELDS__lotr" in str(exc_info.value)
    ...
    def test_artifact_cannot_receive_belongs_to(self, frodo, one_ring):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(frodo, one_ring, "BELONGS_TO__lotr")
        assert "BELONGS_TO__lotr" in str(exc_info.value)
    ...
    def test_race_cannot_receive_from_artifact(self, one_ring, hobbit):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(one_ring, hobbit, "BELONGS_TO__lotr")
        # Artifact can't create BELONGS_TO (outbound fails first)
        assert "cannot create 'BELONGS_TO__lotr' edges" in str(exc_info.value)
    ...
    def test_faction_cannot_receive_allies_from_artifact(self, one_ring, fellowship):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(one_ring, fellowship, "ALLIES_WITH__lotr")
        # Artifact can't create ALLIES_WITH (outbound fails first)
        assert "cannot create 'ALLIES_WITH__lotr' edges" in str(exc_info.value)
...
class TestCrossConstraintValidation:
    ...
    def test_sentinel_to_citadel_blocked_by_inbound(self, sentinel, citadel):
        ...
        with pytest.raises(InvalidEdgeError) as exc_info:
            create_edge(sentinel, citadel, "REFERENCES__lotr")
        assert "cannot receive any inbound edges" in str(exc_info.value)
...
class TestEdgeProperties:
    ...
    def test_edge_with_properties(self, frodo, sting):
        ...
        edge = create_edge(frodo, sting, "WIELDS__lotr", properties={"acquired_at": "Bag End", "is_glowing": True})
    ...
    def test_multiple_edges_same_type(self, frodo, gandalf, sauron):
        ...
        edge1 = create_edge(frodo, sauron, "ENEMIES_WITH__lotr")
        edge2 = create_edge(gandalf, sauron, "ENEMIES_WITH__lotr")
...
class TestEdgePropertyValidation:
    ...
    def test_wields_valid_proficiency(self, frodo, sting):
        ...
        edge = create_edge(frodo, sting, "WIELDS__lotr", properties={"proficiency": "master"})
    ...
    def test_wields_valid_all_fields(self, frodo, sting):
        ...
        edge = create_edge(frodo, sting, "WIELDS__lotr", properties={"proficiency": "apprentice", "primary": True})
    ...
    def test_wields_invalid_proficiency_enum(self, frodo, sting):
        ...
        with pytest.raises(EdgePropertyValidationError):
            create_edge(frodo, sting, "WIELDS__lotr", properties={"proficiency": "expert"})

    def test_wields_invalid_primary_type(self, frodo, sting):
        """WIELDS rejects a non-boolean primary field."""
        with pytest.raises(EdgePropertyValidationError):
            create_edge(frodo, sting, "WIELDS__lotr", properties={"primary": "yes"})
    ...
    def test_mentors_requires_discipline(self, gandalf, frodo):
        ...
        with pytest.raises(EdgePropertyValidationError):
            create_edge(gandalf, frodo, "MENTORS__lotr", properties={})

    def test_mentors_valid_required_only(self, gandalf, frodo):
        """MENTORS succeeds with just the required discipline field."""
        edge = create_edge(gandalf, frodo, "MENTORS__lotr", properties={"discipline": "wizardry"})
    ...
    def test_mentors_valid_all_fields(self, gandalf, frodo):
        ...
        edge = create_edge(gandalf, frodo, "MENTORS__lotr", properties={"discipline": "courage", "duration_years": 17})
    ...
    def test_mentors_rejects_negative_duration(self, gandalf, frodo):
        ...
        with pytest.raises(EdgePropertyValidationError):
            create_edge(gandalf, frodo, "MENTORS__lotr", properties={"discipline": "lore", "duration_years": -1})
    ...
    def test_mentors_rejects_additional_properties(self, gandalf, frodo):
        ...
        with pytest.raises(EdgePropertyValidationError):
            create_edge(gandalf, frodo, "MENTORS__lotr", properties={"discipline": "magic", "secret_technique": "fireball"})
