...
class GenericomOpenAlertsPanelType:
    ...
    def get_view_context(cls, panel: Panel, request: HttpRequest) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(
                "Genericom open-alerts search execution failed for panel %s",
                panel.entity_id,
            )
            return {
                "open_alerts_rows_json": safe_json([]),
...
def _build_rows(envelope: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for n in envelope.get("nodes", []):
        ...
        if parent_id is None:
            logger.warning(
                "Genericom open-alerts: finding %s has no HAS_FINDING parent; skipping",
                finding_id,
            )
            continue
        ...
        if parent is None:
            logger.warning(
                "Genericom open-alerts: HAS_FINDING parent %s for finding %s not in envelope; skipping",
                parent_id,
                finding_id,
            )
            continue
