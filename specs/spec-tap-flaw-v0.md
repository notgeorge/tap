...
- A Flaw carries `flaw_tags`: one or more values from a **registered domain-tag vocabulary**, orthogonal to `flaw_class` (blame) and severity (impact). This is the third routing axis — class → who fixes, tags → which specialty, severity → how urgent.
- v0 vocabulary (illustrative, extensible): `security` (authn/authz/secrets/isolation), `operational` (runtime/availability/jobs), `data` (grid/content integrity), `config` (instance/boot configuration), `integration` (collectors/plugins/upstreams). The auth `unguarded_operation` Flaw is tagged `security`.
