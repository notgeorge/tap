...
1. **Step 0.1 — Host readiness.** The toolchain/layout battery ([req-dev-multisession-host-readiness](#host-readiness-battery)); also detects a first spawn (no registry yet) to set honest build-time expectations (the first image build compiles the FIPS OpenSSL provider, 10–20 minutes; later spawns reuse the cache).
...
7. **Step 4 — Build + start.** `scripts/dc up -d --build`, quiet-captured with a live elapsed counter.
...
- **First-spawn detection:** no `~/tap-sessions/.registry` yet ⇒ first run on this host; used only for honest messaging (the first build's 10–20-minute FIPS OpenSSL compile vs the cached 2–5-minute rebuilds).
