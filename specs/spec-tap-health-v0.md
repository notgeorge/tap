...
- **agreement between the in-code entity-type registry and the `EntityType` DB catalog.** Measured on a healthy booted instance they are 30 and 87 with neither a subset of the other — *different populations*, not a duplication: the catalog carries plugin-declared types, including rows that outlive an evicted plugin. Any probe asserting a relationship would be permanently red. Defining that relationship is the open design question behind duplicate-derivation audit item #7 and must precede anything gating on it.
