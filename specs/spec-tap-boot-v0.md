...
| req-boot-minimal-baseline | [Minimal Core Baseline](#minimal-core-baseline) | Implemented | **Baseline flip landed 2026-07-03.** `core` (zero plugins) is the product baseline; `core_dev` (core + `grid_fixtures`) is the default a plain spawn / the entrypoint boots; `base` is renamed → `test_all` (the permanent test/gate union). Replaces `base = install-everything`, which does not scale and becomes unwritable once plugins live in their own repos. Remaining: the cold-boot gate's **lean-container independence check** (AC-5's second half, `req-dev-validation-lean-boot`) — a genuinely fresh, lean-installed stack that catches core→plugin-dep import leakage (the `requests`/`jwt` class) |
...
| req-boot-minimal-baseline-5 | Retire Base → test_all | Implemented (rename) / Proposed (lean-boot bridge) | `base` is renamed to `test_all`: the **permanent union** the suite runs against (**landed 2026-07-03**). Lean per-profile *test lanes* are infeasible (pytest discovery is file-path — an absent plugin's tests hard-error at collection; `test_settings` sees the installed venv, not a profile), so the union stays. Core independence is bridged by a **lean-container independence gate** that stands up a genuinely fresh, lean-installed stack (own compose project → own venv volume) and full-boots `core`/`core_dev` — the only shape that actually catches core→plugin-dep import leakage (`requests`/`jwt`), since a shared full venv masks it. Tracked as `req-dev-validation-lean-boot` (workstream B, 2026-07-03). A plugin's standalone-test profile is plugin-owned (`plugins/<slug>/*.boot.json`, `spawn --boot-file`), created only on demand. | Coordinate with `req-dev-validation-suite-tiers` + `req-dev-validation-lean-boot` (the gate + Map are the validation session's). |
