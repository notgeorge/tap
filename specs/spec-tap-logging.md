...
- **`FLAW` is the one reserved, cross-cutting code** (the exception to per-producer convention): [`spec-tap-flaw-v0.md`](spec-tap-flaw-v0.md) defines `message_code = FLAW` with a fixed `message_data` shape `{flaw_class, flaw_tags, invariant_id, handling}`, so the "should-never-happen" defect signal has one stable discriminator across every producer. Filtering `message_code = FLAW` selects all Flaws; `flaw_class`/`flaw_tags` in the payload route them. Flaws are structured payloads on this object, not new envelope fields.
