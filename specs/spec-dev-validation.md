...
| Per-profile boot resolution (repo-local `boot/*.boot.json` — samsite excluded since its 2026-08-09 re-home) | `req-dev-validation-smoke-gate` | Per-commit (`pytest`) + pre-push (`cold_boot_gate`) | CI-guarded + Gate-guarded | `tap_boot.guards.profile_resolution` (via `tap/tests/test_guards.py`) |
| Samsite record resolution (in-package, re-homed) | `req-boot-bootstrap-samsite-rehome-4` | Per-commit in the plugin repo (pytest over the installed wheel) | CI-guarded (plugin CI, two-mains) — core's gate deliberately no longer sees this profile | `tap_plugin/samsite/tests/test_boot_record_resolves.py` (digest bijection + schema + cold-resolve incl. collector keys) in `tap-plugin-samsite` |
