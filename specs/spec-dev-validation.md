...
| 2026-08-11 | Multiple additional api-fuzz failures on other runs, same day | `psycopg.errors.UndefinedTable: grid_fixtures__constrained_source` (and the earlier `DeadlockDetected`) in the boot auth/`grid_infra` phase, before any fuzz request | **ROOT-CAUSED + FIXED (session/unified).** Not a fuzz finding and not "cold Postgres concurrency" — a **two-source `INSTALLED_APPS` divergence**: the migrate process built the plugin set from the `TAP_PLUGINS` env var while a sibling `manage.py boot` exec fell back to live `discover_entry_points()`; `importlib.metadata`'s mtime-based FastPath cache disagrees across processes, so the two built different app sets (a type registered without its table migrated → the grant in `search_role._reconcile` throws). Flaky because the divergence is timing-dependent (why the re-run passed). Fix: **`TAP_PLUGINS` is authoritative** — one resolver `preboot.resolved_plugin_app_configs()` (env → entrypoint-persisted `/run/tap-plugins` → warned discovery), collapsing the settings.py/plugin_testing.py copies so migrate and boot cannot diverge. No retry/demotion needed — the class is removed, not tolerated. Follow-ups (DB registry / advisory lock / fail-closed grid invariant) are spec'd separately (the plugin load/update wrapper). |
