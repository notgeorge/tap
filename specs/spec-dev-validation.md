...
| Log-site tokens | `spec-tap-logging.md` (`req-tap-logging-site-id-scanner`) | Bare 4-hex token format + within-file hex uniqueness + baseline ratchet across all committed log calls | Per-commit (`pytest`) | CI-guarded (`tap/guards/log_sites.py` via `tap/tests/test_guards.py`) |
| `record_*` site tokens | `spec-tap-cares-collector.md` | Within-file uniqueness of collector result site tokens | Per-commit (`pytest`) | CI-guarded (`tap/guards/cares.py::RecordSiteUniquenessGuard` via `tap/tests/test_guards.py`) |
