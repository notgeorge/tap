...
| Cold-boot system cycle | this spec (`req-dev-validation-smoke-gate`) | Fresh scratch DB → `createcachetable`+`migrate` from zero → `makemigrations --check` → every shipped profile resolves → strict `base` boot → one real collector cycle through the real backend + in-process drain → scheduler evaluated → PRODUCED_BATCH grid mutation asserted → assembled-instance health, end to end, halt-on-failure. `manage.py cold_boot_gate`, provisioned against a scratch DB by `scripts/gate`; ~66s measured | Pre-push (`scripts/gate`, wired into `promote-to-main.sh`) | Gate-guarded (`tap_boot/management/commands/cold_boot_gate.py`) |
