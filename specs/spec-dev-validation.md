...
| Assembled-instance health | `docs/aar/2026-06-26-tap-cache-latent-provisioning.md` (the pair) | A stood-up instance's real backends actually work: DatabaseCache table provisioned (`tap_cache`), live db + cache set/get round-trip + best-effort queue probe pass. Catches the latent-provisioning-fault class (standup "succeeds" but a never-exercised-at-boot path is broken) | Per-commit (`pytest`) + per-spawn (live `/healthz` gate) | Partially guarded — **CI-guarded** for the unit pieces (`tap_boot/tests/test_checks.py` cache-table system check; `tap/tests/test_health.py` `/healthz` probes) and the database-tagged check fires loud at `migrate`/`check --database`; **per-spawn** the live `/healthz` 200+healthy gate (`spawn-session.sh` Step 6.5) replaces the prior 5xx-passes liveness blindness. The full cold-boot assembled-instance live-health run under automation is **Named, deferred** → folds into the Cold-boot system cycle gate (CI/CD is roadmap item 5, post-July) |
