...
| All-plugins CI lane | `req-dev-validation-all-plugins-lane` | Pre-push (promote-triggered) + CI | Gate-guarded *(target)* — Named, deferred until proven in a real Actions run | `.github/workflows/all-plugins.yml` (boots the `test_all` union, runs the full lane); `promote-to-main.sh` blocks on it (option B) |
