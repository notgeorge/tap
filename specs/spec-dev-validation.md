...
| DCO sign-off trailers (both roads to main) | `req-cicd-dco-signoff` | Pre-push (`scripts/check-dco`, wired into `promote-to-main.sh`) + CI (`product-lines.yml` `dco` job) | Gate-guarded (enforcing since 2026-08-12, when the legal-approved CONTRIBUTING.md + DCO landed at the repo root) | `scripts/check-dco` (every non-merge, non-bot commit added over origin/main carries `Signed-off-by`); the trailer itself is applied by `.githooks/prepare-commit-msg` |
