...
| Per-profile boot resolution | this spec (`req-dev-validation-smoke-gate`) + `spec-tap-boot-v0.md` | Every shipped `boot/*.boot.json` profile's enabled steps resolve against the live registries (seed-plugin slugs/bundles + fire-collector `scope:key`) with zero grid mutation — catches the module-path→slug fire-collector rot class (the 2026-07-02 `samsite` break: the collector-identity refactor repointed grift `SCHEDULED_TARGET` edges but missed the boot-profile keys, silently breaking `boot --profile samsite`, the live-demo profile). `boot --check` is the resolve-only engine | Per-commit (`pytest`) + pre-push (`cold_boot_gate` step `profiles:resolve`) | CI-guarded (`tap_boot/tests/test_shipped_profiles_resolve.py`, incl. a teeth test) + Gate-guarded |
