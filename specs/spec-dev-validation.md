...
| Log-site tokens | `spec-tap-logging.md` (`req-tap-logging-site-id-scanner`) | Bare 4-hex token format + within-file hex uniqueness + baseline ratchet across all committed log calls | Per-commit (`pytest`) | CI-guarded (`tap/guards/log_site_{format,uniqueness,baseline}.py` via `tap/tests/test_guards.py`) |
| `record_*` site tokens | `spec-tap-cares-collector.md` | Within-file uniqueness of collector result site tokens | Per-commit (`pytest`) | CI-guarded (`tap_cares/guards/record_site.py::RecordSiteUniquenessGuard` via `tap/tests/test_guards.py`) |
| Authz coverage | `spec-tap-auth-v0.md` (`req-tap-auth-policy-9`) | Every call to an **undecorated** privileged write sink (`write_batch`/`grift_import`/`_create_node_internal`/`_patch_node_internal`) sits inside a `@requires_capability`/`authorized()` gate. Baseline now **empty/strict** (was 37): the self-gating read executors (`execute_search`/`execute_gryphon_raw`/`explain_gryphon_raw`, each `@requires_capability("grid.read")` at their own def → gated by construction on caller context) were dropped from `SINK_CALLS` as 33/37 false positives (mostly dashboard panels); the 4 real write-sink sites are non-request-reachable (test-only, boot seed, validate CLI) and carry `# TAP-AUTHZ-COV` markers | Per-commit (`pytest`) | CI-guarded (`tap/guards/authz.py` via `tap/tests/test_guards.py`) |
| Direct-write coverage | `spec-tap-auth-v0.md` (`req-tap-auth-policy-9` Rule B) | Every statically-resolvable class-level direct ORM write to a TAP-managed model (`.objects.create`/`.save`/`.delete`) outside the service layer is flagged; the set ratchets to zero (currently empty/strict). The write half of the authz coverage story, complementary to the runtime `tap_grid.write_guard` backstop | Per-commit (`pytest`) | CI-guarded (`tap/guards/direct_write.py` via `tap/tests/test_guards.py`) |
| Service-gateway capability coverage | `spec-grid-service.md` (`req-grid-service-gateway-gated`) | Every public (non-`_`) top-level function in the `tap_grid.services` gateway package carries `@requires_capability`/`@gates_per_operation`; helpers live in `_impl.py` below the gate. Location-scoped (no baseline/heuristic): the complement to authz-coverage's sink heuristic — closes the "which public function is a privileged sink?" judgment call that let the Entity-spine reads ship ungated | Per-commit (`pytest`) | CI-guarded (`tap_grid/guards/service_gateway.py` via `tap/tests/test_guards.py`) |
| Secret leak guard | `spec-tap-cares-secrets.md` (`req-tap-cares-secrets-leak-guard`) | No `*.secret.json` file or envelope-shaped (`scope`+`key`+`kind`+`data`) secret enters the repo tree — push-protection beyond `.gitignore`, excluding the mount + vendored dirs | Per-commit (`pytest`) | CI-guarded (`tap/guards/secret_leak.py::SecretLeakGuard` via `tap/tests/test_guards.py`; pure-scan unit tests in `tap/tests/test_secret_leak_scan.py`) |
...
| Gryphon fuzz-campaign ledger | `plugins/gryphon_playground/specs/spec-gridkin-v0.md` (`req-gridkin-fuzz-campaign`) | Long-soak complement to the property fuzzer: grinds a large FRESH seed band (advanced past every prior campaign), records distinct-new-defects-per-100k over fresh space + the oracle-asserted fraction to an append-only ledger, so bug frequency can be watched trending down as the executor hardens. Measurement, not a gate | On-demand script, loopable for hours (`scripts/gryphon-fuzz-campaign`) | Manual (CI-unguarded by design) — the gate is the property fuzzer; the campaign is a trend instrument. Ledger `gridkin/fuzz-campaign-log.jsonl` is committed |
| Gryphon findings ledger (bug locality) | `plugins/gryphon_playground/specs/spec-gridkin-v0.md` (`req-gridkin-findings-ledger`) | Locality sibling of the campaign: one human-authored row per root-caused executor/oracle bug records WHICH subsystem/functions the defect lived in, so concentration points become deliberate refactor targets. The hotspot read pairs finding-count with line coverage (honest denominator). Well-formedness/vocabulary is gated; the analysis is a manual read | Fix-time append (discipline) + on-demand report (`scripts/gryphon-findings`) | Split: CI-guarded for ledger well-formedness + controlled vocabulary (`plugins/gryphon_playground/tests/test_gryphon_findings_ledger.py`); Manual (by design) for the hotspot analysis. Ledger `gridkin/gryphon-findings.jsonl` is committed |
| Cold-boot system cycle | this spec (`req-dev-validation-smoke-gate`) | Fresh scratch DB → `createcachetable`+`migrate` from zero → `makemigrations --check` → every shipped profile resolves → strict `base` boot → one real collector cycle (the offline `grid_fixtures:canary`) through the real backend + in-process drain → scheduler evaluated → PRODUCED_BATCH grid mutation asserted → assembled-instance health, end to end, halt-on-failure. `manage.py cold_boot_gate`, provisioned against a scratch DB by `scripts/gate`; ~66–107s measured | Pre-push (`scripts/gate`, wired into `promote-to-main.sh`) | Gate-guarded (`tap_boot/management/commands/cold_boot_gate.py`) |
| Per-profile boot resolution | this spec (`req-dev-validation-smoke-gate`) + `spec-tap-boot-v0.md` | Every shipped `boot/*.boot.json` profile's enabled steps resolve against the live registries (seed-plugin slugs/bundles + fire-collector `scope:key`) with zero grid mutation — catches the module-path→slug fire-collector rot class (the 2026-07-02 `samsite` break: the collector-identity refactor repointed grift `SCHEDULED_TARGET` edges but missed the boot-profile keys, silently breaking `boot --profile samsite`, the live-demo profile). `boot --check` is the resolve-only engine | Per-commit (`pytest`) + pre-push (`cold_boot_gate` step `profiles:resolve`) | CI-guarded (`tap_boot/guards/profile_resolution.py::ProfileResolutionGuard` via `tap/tests/test_guards.py`; teeth in `tap_boot/tests/test_shipped_profiles_resolve.py`) + Gate-guarded |
| Migration completeness (`makemigrations --check`) | this spec (`req-dev-validation-smoke-gate`) | No model change lacks a migration on the tree that becomes `origin/main` (the #1 Django table-stakes gap both NetBox/Nautobot close and TAP lacked) | Pre-push (`cold_boot_gate` step `schema:makemigrations`) | Gate-guarded |
...
| Schedule grift target integrity | `spec-tap-cares-collector.md` (`req-tap-cares-collector-model-10`) | Every `SCHEDULED_TARGET` edge hardcoded in a shipped plugin grift bundle resolves to a registered collector's derived on-grid id (`uuid5("{scope}:{key}")`) — catches stale hardcoded ids / scope-key drift that dangle the edge and abort boot's population phase on a fresh spawn (the package-mode-migration failure class: a collector's module path changed and a hardcoded grift target went stale). Discovers plugins generically, so tap_cares carries no per-plugin dependency | Per-commit (`pytest`) | CI-guarded (`tap_cares/guards/schedule_grift.py::ScheduleGriftTargetsGuard` via `tap/tests/test_guards.py`) |
| JSON-file naming | `spec-tap-json-files.md` (`req-tap-json-naming`) | Every TAP-owned `.json` file's name declares its role — discovered data/config `<name>.<role>.json` (role in {boot, secret, grift, edge, gridkin}), singleton app config `<app>.<name>.json`, or schema `<type>.schema.json` — so the role-keyed loaders can discover it and a reader can see its purpose; grandfathered names sit in a baseline that ratchets to zero | Per-commit (`pytest`) | CI-guarded (`tap/guards/json_naming.py::JsonNamingGuard` via `tap/tests/test_guards.py`) |
| Recurring-task uniqueness | `spec-tap-cares-task-backend.md` (`req-tap-cares-task-backend-recurring-scope-4`) | Exactly one Steady Queue `@recurring` decoration exists (the TAP scheduler tick); every other scheduling need routes through an on-grid `Schedule` entity (auditable, FLIP-able), never a new code-level recurring task | Per-commit (`pytest`) | CI-guarded (`tap_cares/guards/recurring.py::RecurringUniquenessGuard` via `tap/tests/test_guards.py`) |
| Static typing (mypy) | this spec (`req-dev-validation-mypy-ratchet`) | `mypy .` strict-mode error set is frozen per file+error-code and ratchets down; a NEW error (new untyped code, a genuine new `union-attr`/None-access) fails at authoring time. The ~1200 baselined are audited debt — django-stubs dynamic-ORM friction + test-fixture `no-untyped-call` cascade, not latent bugs — so the value is forward (block new), not the clean-to-zero diff. Also fixes the package-mode namespace-plugin resolution (`exclude` + `py.typed` markers) that made `mypy .` collide/abort | Per-commit (`pytest`, ~4s warm) | CI-guarded (`tap/guards/mypy.py::MypyRatchet` via `tap/tests/test_guards.py`) |
...
| req-dev-validation-smoke-gate-2 | Seed is strict | Implemented | Strict seed; any failed bundle fails the gate. | Step `seed:boot-base` runs the real `base` boot (population seeds through the same `seed_plugin` op `import_plugin_grift` uses); aligns with `req-dev-multisession-spawn-import-strict-1`. |
...
| req-dev-validation-smoke-gate-7 | Every shipped profile resolves | Implemented | The gate cold-resolves every shipped boot profile against the live registries (per-profile axis); a rotted fire-collector key or missing plugin/bundle fails the gate. | Step `profiles:resolve` + per-commit `test_shipped_profiles_resolve.py`. Caught + fixed the live `samsite` break on build. |
