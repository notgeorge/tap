...
| req-dev-validation-product-line-lanes | [Per-Product-Line CI Lanes](#per-product-line-ci-lanes) | Proposed | Validate each product line (a plugin-pack + boot profile) on its own AWS CodeBuild GitHub Actions runner, in parallel across lines — parallelism along the product axis, not arbitrary shards. In-account IAM gives each lane native Bedrock / `aws_core` capability. The active CI direction; supersedes free-runner sharding. |
...
| All-plugins CI lane | `req-dev-validation-all-plugins-lane` | Pre-push (promote-triggered) + CI | Gate-guarded — lane PROVEN GREEN in a real Actions run; promote wiring landed but unexercised until the first post-bootstrap promote | `.github/workflows/all-plugins.yml` (boots the `test_all` union, runs the full lane); `promote-to-main.sh` Step 2.6 blocks on it (option B, req-dev-multisession-ci-gate) |
...
| Sub-RID | Name | Status | Acceptance | Notes |
| --- | --- | :---: | --- | --- |
| req-dev-validation-product-line-lanes-1 | One lane per product line | Proposed | Each product line (profile) has a lane that boots *that* profile and runs *that* line's tests; lines run in parallel. Adding a line = one Terraform `product_lines` entry + one workflow matrix entry. | Meaningful (product) axis. `test_all` is the union superset lane. |
| req-dev-validation-product-line-lanes-2 | CodeBuild GHA runner | Proposed | Each lane runs on an AWS CodeBuild project registered as a GitHub Actions runner (EC2-mode, `privileged_mode` for docker-in-docker). Managed service — no AMI/NAT/autoscaler. | `ci/terraform/codebuild-runners/main.tf`. |
| req-dev-validation-product-line-lanes-3 | Per-profile template clone | Proposed | Each lane builds a per-profile pre-migrated template DB and workers clone it (`TAP_TEST_DB_TEMPLATE`), eliminating the per-worker migrate-from-zero race + speeding setup. | `scripts/build-test-template`; `tap/test_settings.py`. |
| req-dev-validation-product-line-lanes-4 | In-account capability, least-privilege per line | Proposed | Each lane's IAM role is per-line (grants can diverge) and grants only what that line tests need — native Bedrock and/or scoped `aws_core` STS. The AWS-native reason to run CI here. | `ci/terraform/codebuild-runners/iam.tf`; reuse the External-ID discipline from `plugins/aws_core/.../handoff/cross-account-role.yaml`. |
| req-dev-validation-product-line-lanes-5 | IaC in-repo, state out | Proposed | The provisioning is Terraform tracked in-repo; state + real tfvars (ARNs/account ids) are gitignored, never committed. | `ci/terraform/codebuild-runners/.gitignore`, `terraform.tfvars.example`. |
| req-dev-validation-product-line-lanes-6 | Promote-gate + Map row when live | Proposed | Once the lanes run green, wire them into the promote gate (reciprocal of `req-dev-multisession-ci-gate`) and add their Validation Map row(s) via `DECLARED_SURFACES` (`req-dev-validation-map-3`). Deferred until the CodeConnections auth + first green run. | Not a live surface yet, so no Map row is claimed prematurely. |
...
- **Per-product-line CodeBuild lanes** — no longer out of scope: promoted to the active requirement [`req-dev-validation-product-line-lanes`](#per-product-line-ci-lanes) (Proposed), superseding the parked free-runner sharding. Artifacts authored (`ci/terraform/codebuild-runners/`, `.github/workflows/product-lines.yml`); blocked on the AWS-side CodeConnections authorization + first apply. GitHub's postponed-not-cancelled 2026 $0.002/min self-hosted charge is the standing tail risk.
