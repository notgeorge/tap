...
- **AWS-native (self-hosted) CI runner.** Standing up self-hosted GitHub runners inside AWS is deferred — **not** as a speed/cost lever (matrix-sharding on free runners, `req-dev-validation-all-plugins-lane-7`, is faster and ~$5/mo; baseline CI is ~free and AWS adds a permanent ops surface). Its only unique value is a **capability axis, not a speed axis**: a runner carrying an IAM role can test Bedrock LLM calls and `aws_core` STS/AssumeRole paths natively (role-native, no long-lived creds in GH secrets). **Trigger: the first test that genuinely must execute inside AWS** — a real Bedrock or live-`aws_core` integration test — not a wish to make CI faster. When it fires: `github-aws-runners/terraform-aws-github-runner`, ephemeral spot runners, pre-baked AMI (Docker+Postgres warm), least-privilege role reusing the `aws_assumed_role` External-ID discipline. Also note GitHub's postponed-not-cancelled 2026 $0.002/min self-hosted charge as a tail risk. Full evaluation: [doc-dev-validation-ci-runner-strategy.md](../docs/misc/doc-dev-validation-ci-runner-strategy.md).
