...
| Authz coverage | `spec-tap-auth-v0.md` (`req-tap-auth-policy-9`) | Every call to a privileged graph sink (`write_batch`/`grift_import`/Search+Gryphon executors/`_*_internal`) sits inside a `@requires_capability`/`authorized()` gate; baseline ratchets to zero | Per-commit (`pytest`) | CI-guarded (`tap/guards/authz.py` via `tap/tests/test_guards.py`) |
