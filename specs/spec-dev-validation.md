...
- **Per-product-line CodeBuild testing lanes (`req-dev-validation-product-line-lanes`, Proposed — the active direction + DoD).** The chosen destination (decided 2026-07-08), superseding free-runner sharding (`req-dev-validation-all-plugins-lane-7`, Parked). TAP's products *are* boot profiles: a product line = a plugin-pack + boot profile (samsite, a FedRAMP-specific line, a customer-specific line). CI should validate **per product line** — one **AWS CodeBuild** GitHub Actions runner project per line, each booting *that line's profile*, running *that line's tests*, in parallel across lines. This parallelizes along a *meaningful* axis (product boundaries), each lane validates a real deliverable, it scales with the business (new line = new lane), and it sidesteps the monolithic-`test_all` sharding flakiness (each lane is a deterministic, smaller profile). CodeBuild is the vehicle because it is a **managed AWS service** (no AMI/NAT/autoscaler), EC2-mode compute supports docker-in-docker, and it runs **in our account** → an IAM role for native Bedrock / `aws_core` STS testing (the capability axis, no long-lived creds) — *not* a hand-built `terraform-aws-github-runner` EC2 fleet, and *not* a managed SaaS runner (WarpBuild/Blacksmith give speed but no in-account identity). Each lane clones a per-profile pre-migrated template (`scripts/build-test-template` + `TAP_TEST_DB_TEMPLATE`) for fast, race-free setup. `test_all` remains one lane (the union superset). GitHub's postponed-not-cancelled 2026 $0.002/min self-hosted charge is a tail risk. Full evaluation incl. the non-EC2 lanes table: [doc-dev-validation-ci-runner-strategy.md](../docs/misc/doc-dev-validation-ci-runner-strategy.md).
