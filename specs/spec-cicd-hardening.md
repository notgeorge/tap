...
| req-cicd-dco-signoff | [DCO Sign-Off Enforcement](#dco-sign-off-enforcement) | Implemented | Auto-applied `Signed-off-by` trailer (versioned hook) + ENFORCING trailer check on both roads to main since 2026-08-12, when the legal-approved CONTRIBUTING.md + DCO landed at the repo root. |
...
| req-cicd-dco-signoff-3 | Enforcement flips with the policy | Implemented | Landed 2026-08-12: legal-approved `CONTRIBUTING.md` + `DCO` at the repo root, and `scripts/check-dco` now FAILS on a missing trailer — the enforcing default lives in the script itself (escape hatch `TAP_DCO_REPORT_ONLY=1`) rather than an env var per invoker, so an ad-hoc run can never be quieter than the gate. | Pre-flight before flipping: no open PR and no live session branch carried an unsigned commit (two 3-week/3-month-stale branches did; both long superseded). The red spells out both remediation forms (`--amend -s`, `rebase --exec`) and the hooksPath caveat for plain clones. Composes with `req-cicd-branch-protection`: under mandatory PRs the `dco` job becomes a required check. |
