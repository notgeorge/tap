...
| req-cicd-crypto-bom | Crypto Bill-of-Materials gate — every provider, not just OpenSSL | **Implemented (scan + per-plugin conformance + manifest [fips] declaration + boot-time global gate + operator waivers) 2026-07-21 · per-runtime probes pending** | The FIPS invariant is **every cryptographic *provider* that can execute in the deployed artifact is the validated module (system OpenSSL #4282) or that ecosystem's validated equivalent — or is proven unreached, or explicitly named out-of-boundary.** OpenSSL is only what *Python* uses; a Go binary, a Rust crate on `ring`/`aws-lc-rs`, a `libsodium`/`pynacl` wheel, or a JVM's BouncyCastle each carries its OWN crypto that ignores `OPENSSL_CONF` and would **silently** run non-FIPS crypto with no error — the blind spot the `tap.fips` boot self-check (OpenSSL-only) cannot see (doc-fips-assessment-record.md L17). **Layer 1 (built):** `tap.crypto_bom` fingerprints every ELF artifact in an environment (core venv + image binaries, or a single plugin's closure) for crypto-provider byte-signatures, classifies each against the curated `tap.crypto_providers` registry, and **fails-closed** on any provider with no disposition or a `must-fix` one. Dispositions are VALIDATED / OUT-OF-BOUNDARY (provisioning, e.g. uv's aws-lc-rs at install time) / UNREACHED (e.g. gosu's dormant Go crypto/tls) / MUST-FIX. **Positioned for plugins:** `scan()` takes explicit roots so per-plugin conformance scans a plugin's isolated closure — a plugin leaking `pynacl` or a Go collector defeats a FIPS-capable core (plugins run in the same image/process). The core gate runs under `test_all` (the full plugin union). Scope: Python + subprocess Go tools + Rust extensions (JVM/BouncyCastle detected-but-out-of-scope). Anti-fail-open: the gate asserts it actually read binaries + saw known providers (L2/L12). Validation Map row under this RID. **Global validation + operator waivers (built 2026-07-21):** the FIPS-vs-plugins policy is **declare vs decide** — a plugin AUTHOR cannot excuse itself from a system security property (a careless/hostile plugin marking itself exempt is the leak). So: (1) **per-plugin conformance** (`validate_plugin` `crypto-providers` check) scans a plugin's shipped artifacts + declared deps and REPORTS its posture (a warning — a plugin may legitimately use non-FIPS crypto in a non-FIPS deployment; `--strict` conformance CI escalates); (2) the **boot-time system gate** (`python -m tap.crypto_bom --gate`, wired in `docker/entrypoint.sh` after `tap.fips`) enforces GLOBALLY when `TAP_FIPS_MODE=1` — it scans the whole assembled environment (core + every installed plugin) and refuses to serve (`TAP-ABORT`) if any provider is non-validated; (3) the **operator waiver** is the escape valve — `fips_waivers` in the boot profile (deployment-controlled, NOT the plugin's manifest), each entry naming plugin/artifact + provider + a **mandatory reason** (a blank reason is rejected — you cannot waive silently), surfaced as WAIVED so every exception is auditable. | **Manifest `[fips]` declaration (built 2026-07-21, `req-plugin-manifest-v0-fips`):** the "declare" half of declare-vs-decide — an optional `[fips]` table in `tap-plugin.toml` where the author states `compatible` or `uses-nonvalidated` (+ a mandatory reason), VERIFIED against the scan by the `crypto-providers` conformance check: a false `compatible` FAILS, an honest `uses-nonvalidated` PASSES (letting a legitimately-non-FIPS plugin pass `--strict`), an undeclared leak WARNs. First dogfood: grid_fixtures declares `compatible`. **Pending:** (a) **per-runtime runtime self-checks** — `tap.fips` is the Python/OpenSSL instance of a general pattern; a Go binary in-boundary needs `GODEBUG=fips140=on` + self-test, a JVM needs BC-FIPS providers, Rust needs aws-lc-rs FIPS mode; run only for runtimes actually present. (c) **The uv provisioning-boundary decision** is recorded OUT-OF-BOUNDARY with a rationale; if an assessor rules provisioning in-boundary, enable uv's aws-lc-rs FIPS build (aws-lc IS CMVP-validated). (d) The DB image (gosu) is dispositioned but its scan runs in the db container, not the web gate — wire a db-image scan. |
