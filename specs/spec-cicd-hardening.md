...
- **FIPS is decided** (`-5`/`-6`): self-built OpenSSL 3.0 #4282 provider, no vendor module, **on by default**. The one thing that could reopen it is the **3PAO rejecting vendor-affirmed OE portability** on Wolfi — then a validated-OE base becomes necessary, and the already-proven `ubi-micro` + `dnf --installroot` path is the landing spot (accepting RHEL's **host-derived** FIPS, i.e. the deployment host must run `fips=1`). Confirm this acceptance early; it is the single highest-leverage open question in this requirement.
