...
| --- | --- | --- | --- | --- | --- |
| authz (`tap/authz_coverage.py`) | ✅ ratchet | per-function | anchor `path::qualname::sink` | anchor ✅ / collapses early ❌ | de-collapse in `measure()` (`req-tap-callsite-identity-scan-rich-collapse-late`); keep the anchor; add discriminator for SARIF |
| direct-write (`tap/direct_write_coverage.py`) | ✅ ratchet | per-call | `path:lineno` | ❌ churns; no qualname captured | enrich scanner (scope stack + `Model.op`); baseline key → occurrence_key `path::qualname::Model.op#<disc>`; re-baseline |
...
| log-site — violation baseline (`tap/guards/log_site_baseline` via `_log_site_scan.py`) | ✅ ratchet | per-call | `path:lineno` (`key()`) | ❌ line-keyed (baseline currently empty, so latent) | a missing/malformed site has no `[<hex>]` yet, so keying by `path::[<none>]` would collapse distinct missing-token calls in one file. Key each violation as a per-call occurrence key `path::qualname::logger.<level>::<violation-kind>#<disc>`; a site graduates to the well-formed `path::[<hex>]` anchor once a real token is minted |
