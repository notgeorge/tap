...
def test_healthy_when_all_healthy(isolated_probe_registry):
    ...
    register_health_probe("b", ProbeResult.healthy)
    report = run_health()
...
def test_critical_unhealthy_flips_overall(isolated_probe_registry):
    ...
    register_health_probe("ok", ProbeResult.healthy)
    report = run_health()
...
def test_noncritical_unhealthy_is_degraded_not_hidden(isolated_probe_registry):
    ...
    register_health_probe("soft", lambda: ProbeResult.unhealthy("x.soft"), critical=False)
    register_health_probe("ok", ProbeResult.healthy, critical=True)
    report = run_health()
...
def test_degraded_when_any_degraded_no_critical_unhealthy(isolated_probe_registry):
    register_health_probe("d", lambda: ProbeResult.degraded("x.deg"))
    register_health_probe("ok", ProbeResult.healthy, critical=True)
    assert run_health().status is ProbeStatus.DEGRADED


@pytest.mark.spec("req-tap-health-probes-3")
def test_unknown_visible_but_does_not_flip(isolated_probe_registry):
    register_health_probe("u", lambda: ProbeResult.unknown("x.unknown"), critical=False)
    register_health_probe("ok", ProbeResult.healthy, critical=True)
    report = run_health()
...
def test_probe_exception_is_isolated(isolated_probe_registry):
    ...
    register_health_probe("boom", boom, critical=True)
    register_health_probe("fine", ProbeResult.healthy)
    report = run_health()  # must not raise
