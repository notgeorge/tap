...
- **LANDMINE (v0, fix deferred): `grift_import` rejects a batch ATOMICALLY on one hard error (e.g. a single `duplicate_entity_id`). A rejected batch is in NEITHER `result.imported_batches` NOR `skipped_batches` — ONLY `result.errors`.** `CollectorBase.submit_grift` forwards only imported/skipped and ignores `result.errors`, so any collector / `submit_grift` / `grift_import` caller that trusts `imported`/`batches_imported` and doesn't inspect `result.errors` **silently loses the entire batch, records 0 errors, and reports the run successful** (confirmed live 2026-05-19; canned tests stay green because atomic rejection only triggers on real-data shapes). Mandatory until GRIFT is hardened: any such caller MUST check `result.errors` and fail loud; treat "0 imported, 0 errors, non-empty batch" as a red flag, never success. Only the aws_core boto3 collector (`GRIFT_BATCH_REJECTED`) is currently guarded; every other caller is exposed.
