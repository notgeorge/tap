...
update-triggers:
  ...
  - The opt-in searchability gate lands (the DB grant narrows from the full registered-type registry to the searchable subset — update the readonly-role scope caveat throughout).
...
| **DB-denies** (defense in depth) | Least-privilege `tap_gryphon_ro` DB role: `SELECT` on exactly the registry-derived grid tables + spine, nothing else. `search_readonly` authenticates as it. A read reaching a non-grid table is denied by PostgreSQL regardless of any in-code guard. Provisioned idempotently in a boot grid-infra phase; grant set derived from the type registry so it cannot drift. | `req-grid-search-readonly-role.sec` (+ sec-1/2/3/4/6), `req-boot-search-role` (+ sec-1..6) | `23e63897` |
...
[3] SCOPE-NARROW    opt-in searchability gate ..................... DEFERRED  (exec-searchable.sec)
   │               (grant = FULL registry until this lands)
   ▼
