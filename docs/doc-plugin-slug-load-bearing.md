...
| **Owner-namespaced entity types** `<slug>__<name>` (nodes), `NAME__<slug>` (edges), tables `<slug>__*` | `req-plugin-arch-isolation`, `spec-plugin-type-ownership-v0` — **Proposed as a systematic requirement; LIVE for early adopters.** `grid_fixtures` ships `grid_fixtures__leaf`/`…__hub`/`…__node`/`…__cycle_node` + `PG_LINKS__grid_fixtures` today; existing plugins (e.g. `aws_account`) remain author-chosen | for an adopter this is **already** a data + string-reference identity: a slug rename is a data migration whose *dominant* cost is string references to the type slugs (test corpora, GRIFT/fixture/expected data, Gryphon query strings, cross-plugin edge endpoints), not the model files — per that spec's own cost model. `grid_fixtures`' Gridkin corpus references `grid_fixtures__*` by string, so it is the first plugin for which the slug is load-bearing on-grid identity. |
