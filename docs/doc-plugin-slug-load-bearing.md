...
| --- | --- | --- |
| **Secret paths** `tap_plugin/<slug>/…` (a plugin finds its own secrets by slug; `scope` = owner namespace, key authored) | `req-tap-cares-secrets-consumer-scoping` (consumer-first `scope` = `tap_plugin/<slug>`) Proposed; `req-plugin-arch-source-secret` (the install credential, infra-scoped `tap_plugins/source`) Implemented 2026-07-03 | silently relocates operator-provisioned secret files — an external contract. This is why the `scope` anchors on the slug (stable, gated) and never on `__name__`. |
