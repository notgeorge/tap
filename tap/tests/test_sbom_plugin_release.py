...
def test_identity_gate_passes_on_matching_wheel() -> None:
    ...
    assert plug.check_plugin_identity(doc, "tap-plugin-aws-core", "0.4.1") == []
...
def test_identity_gate_catches_version_mismatch() -> None:
    ...
    doc = _wheel_cdx("tap-plugin-aws-core", "0.4.1")
    problems = plug.check_plugin_identity(doc, "tap-plugin-aws-core", "0.4.2")
...
def test_identity_gate_serves_non_plugin_dists() -> None:
    ...
    assert plug.check_plugin_identity(doc, "aws-secrets-source", "0.2.0") == []
    problems = plug.check_plugin_identity(doc, "aws-secrets-source", "0.3.0")
    assert any("version mismatch" in p for p in problems)
...
def test_identity_gate_catches_absent_plugin_component() -> None:
    ...
    problems = plug.check_plugin_identity(doc, "tap-plugin-aws-core", "0.4.1")
