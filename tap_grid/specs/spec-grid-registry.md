...
A lighter near-term affordance, consistent with the permissive-runtime stance, is **detection rather than enforcement**: a dev-validation check that *surfaces* same-slug-different-plugin registrations for human review (flagging likely homonyms — disjoint source/target domains, divergent name/description — vs. compatible co-extension), with a small allowlist for the genuinely shared edges. This belongs with the backlogged plugin-dependency model (`spec-tap-testing.md` — "graduate from review-time discipline into a designed plugin-dependency model"), which is the natural home for declared ownership, narrow contracts, and detectable breakage. Defer the structural work to the plugin refactor; the detection net is the cheap interim guard.
