...
- **`Collector`** (current): `tap_cares.registry.register_collector(...)` registers the runner class read-only at `ready()`; `tap_cares.registry.reconcile_collector_nodes()` materializes the on-grid `Collector` node afterward under a bound actor (deferred so `ready()` stays read-only per `req-plugin-load-v0-ready-readonly`).
